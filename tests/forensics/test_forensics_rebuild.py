...
class TestForensicsRebuild(unittest.TestCase):
    def setUp(self):
        ...
        self.test_pcap = os.path.join(self.temp_dir.name, "test_forensics.pcap")
        self.engine = ForensicsEngine(data_dir=self.temp_dir.name, silent=True)
    ...
    def test_beaconing_detection(self):
        ...
        for i in range(25): # More packets for better stats
            p = Ether()/IP(src="192.168.1.50", dst="8.8.8.8")/TCP(sport=1234, dport=4444)/bytes(64)
            p.time = start_time + i * 10.0
            packets.append(p)
    ...
    def test_dns_entropy(self):
        ...
        for index in range(20):
            ...
            packet = IP(src="192.168.1.20", dst="8.8.8.8")/UDP(dport=53)/DNS(qd=DNSQR(qname=query, qtype="TXT"))
            packet.time = float(index)
            alerts.extend(dns_detector.detect(domain=query, packet=packet))
    ...
    def test_identity_extraction(self):
        ...
        user_name = "analyst".encode('utf-16le')
        ...
        # The valid NTLM observation is retained; the Kerberos-like opaque bytes
        # do not contain a complete realm/principal and must not overwrite it.
        self.assertEqual(profile.user, "analyst")
    ...
    def test_multi_suspect_profiles(self):
        ...
        start_time = time.time()
        for i in range(20):
            p = Ether(src="00:11:22:33:44:55")/IP(src="192.168.1.10", dst="8.8.8.8")/TCP(sport=1234, dport=4444)/bytes(64)
            p.time = start_time + i * 10.0
            packets.append(p)
        ...
        for index in range(20):
            label = f"a0b1c2d3e4f5g6h7i8j9k{index:02x}q"
            p2 = Ether(src="AA:BB:CC:DD:EE:FF")/IP(src="192.168.1.20", dst="8.8.8.8")/UDP(sport=53000, dport=53)/DNS(rd=1, qd=DNSQR(qname=f"{label}.tunnel.example", qtype="TXT"))
            p2.time = start_time + index
            packets.append(p2)
